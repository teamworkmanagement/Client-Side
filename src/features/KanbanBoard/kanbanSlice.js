...
const kanbanSlice = createSlice({
  ...
  reducers: {
    ...
    signalRAddNewTask(state, action) {
      ...
      const list = state.kanbanBoard.kanbanLists.find(
        (x) => x.kanbanListId === action.payload.kanbanListId
      );
      if (list) {
    ...
    signalRAddNewList(state, action) {
      ...
      if (
        action.payload.kanbanListBoardBelongedId ===
        state.kanbanBoard.currentBoard
      ) {
        console.log("trùng");
    ...
    signalRRemoveTask(state, action) {
      ...
      if (list) {
        ...
        const index = list.taskUIKanbans.findIndex(
    ...
    signalRRemoveList(state, action) {
      ...
      if (
        ...
        const index = state.kanbanBoard.kanbanLists.findIndex(
          (e) => e.kanbanListId === action.payload.kanbanListId
        );
    ...
    signalRMoveTask(state, action) {
      ...
      if (state.kanbanBoard.currentBoard === action.payload.boardId) {
        ...
        if (action.payload.oldList === action.payload.newList) {
          const listTasks = state.kanbanBoard.kanbanLists.find(
            (x) => x.kanbanListId === action.payload.newList
          );
          const task = listTasks.taskUIKanbans.find(
            (x) => x.taskId === action.payload.taskId
          );
          console.log(task);
          task.taskRankInList = action.payload.position;
          listTasks.taskUIKanbans.sort((x, y) =>
            x.taskRankInList > y.taskRankInList ? 1 : -1
          );
        } else {
          const listTasksOld = state.kanbanBoard.kanbanLists.find(
            (x) => x.kanbanListId === action.payload.oldList
          );

          const index = listTasksOld.taskUIKanbans.findIndex(
            (x) => x.taskId === action.payload.taskId
          );

          const task = {
            ...listTasksOld.taskUIKanbans[index],
            taskRankInList: action.payload.position,
            kanbanListId: action.payload.newList,
          };
          listTasksOld.taskUIKanbans.splice(index, 1);

          const listTasksNew = state.kanbanBoard.kanbanLists.find(
            (x) => x.kanbanListId === action.payload.newList
          );

          listTasksNew.taskUIKanbans.push(task);

          listTasksNew.taskUIKanbans.sort((x, y) =>
            x.taskRankInList > y.taskRankInList ? 1 : -1
          );
        }
      } else {
    ...
    signalRMoveList(state, action) {
      ...
      if (action.payload.kanbanBoardId === state.kanbanBoard.currentBoard) {
        ...
        const obj = state.kanbanBoard.kanbanLists.find(
          (e) => e.kanbanListId === action.payload.kanbanListId
        );
        obj.kanbanListRankInBoard = action.payload.position;
        state.kanbanBoard.kanbanLists.sort((x, y) =>
          x.kanbanListRankInBoard > y.kanbanListRankInBoard ? 1 : -1
        );
      } else {
        state.signalrData.moveList = null;
      }
    ...
    signalRUpdateTask(state, action) {
      ...
      if (list) {
        ...
        const obj = list.taskUIKanbans.find(
          (x) => x.taskId === action.payload.taskId
        );
    ...
    reAssignUser(state, action) {
      ...
      if (list) {
        ...
        const task = list.taskUIKanbans.find(
          (x) => x.taskId === action.payload.taskId
        );
    ...
    signalRAddFile(state, action) {
      ...
      if (list) {
        ...
        const task = list.taskUIKanbans.find(
          (x) => x.taskId === action.payload.fileTaskOwnerId
        );
        task.filesCount++;
      } else {
        state.signalrData.addNewFile = null;
  ...
  extraReducers: {
    [getBoardDataForUI.fulfilled]: (state, action) => {
      console.log(action.payload);
      state.kanbanBoard.kanbanLists = action.payload
        ? action.payload.kanbanListUIs
        : [];
      state.kanbanBoard.currentBoard = action.payload
        ? action.payload.kanbanBoardId
        : null;
